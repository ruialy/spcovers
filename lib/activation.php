...
function roots_theme_activation_options_add_page() {
	...
	if ( ! $roots_activation_options ) {
		...
		if ( is_admin() && isset( $_GET['page'] ) && 'theme_activation_options' === $_GET['page'] ) {
			flush_rewrite_rules();
...
function roots_theme_activation_options_render_page() { ?>
  <div class="wrap">
    <h2><?php printf(__('%s Theme Activation', 'tend'), wp_get_theme()); ?></h2>
    <div class="update-nag">
        <p><?php _e('These settings are optional and should usually be used only on a fresh installation', 'tend'); ?></p>
        <p><strong><?php _e('After theme activation, find and replace ",\'tend\' " to set the textdomain and replace \'tend\' in style.css', 'tend'); ?></strong></p>
    </div>
    <?php settings_errors(); ?>
    ...
    <form method="post" action="options.php">
      ...
      <table class="form-table">
        <tr valign="top"><th scope="row"><?php _e('Create static front page?', 'tend'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Create static front page?', 'tend'); ?></span></legend>
              <select name="roots_theme_activation_options[create_front_page]" id="create_front_page">
                <option selected="selected" value="true"><?php echo _e('Yes', 'tend'); ?></option>
                <option value="false"><?php echo _e('No', 'tend'); ?></option>
              </select>
              <p class="description"><?php printf(__('Create a page called Home and set it to be the static front page', 'tend')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Change permalink structure?', 'tend'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Update permalink structure?', 'tend'); ?></span></legend>
              <select name="roots_theme_activation_options[change_permalink_structure]" id="change_permalink_structure">
                <option selected="selected" value="true"><?php echo _e('Yes', 'tend'); ?></option>
                <option value="false"><?php echo _e('No', 'tend'); ?></option>
              </select>
              <p class="description"><?php printf(__('Change permalink structure to /&#37;postname&#37;/', 'tend')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Create navigation menu?', 'tend'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Create navigation menu?', 'tend'); ?></span></legend>
              <select name="roots_theme_activation_options[create_navigation_menus]" id="create_navigation_menus">
                <option selected="selected" value="true"><?php echo _e('Yes', 'tend'); ?></option>
                <option value="false"><?php echo _e('No', 'tend'); ?></option>
              </select>
              <p class="description"><?php printf(__('Create the Primary Navigation menu and set the location', 'tend')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Add pages to menu?', 'tend'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Add pages to menu?', 'tend'); ?></span></legend>
              <select name="roots_theme_activation_options[add_pages_to_primary_navigation]" id="add_pages_to_primary_navigation">
                <option selected="selected" value="true"><?php echo _e('Yes', 'tend'); ?></option>
                <option value="false"><?php echo _e('No', 'tend'); ?></option>
              </select>
              <p class="description"><?php printf(__('Add all current published pages to the Primary Navigation', 'tend')); ?></p>
            </fieldset>
          </td>
        </tr>
      </table>
...
function roots_theme_activation_action() {
  ...
  if ($roots_theme_activation_options['create_front_page'] === 'true') {
    ...
    $default_pages = array(__('Home', 'tend'));
    ...
    $home = get_page_by_title(__('Home', 'tend'));
  ...
  if ($roots_theme_activation_options['create_navigation_menus'] === 'true') {
    ...
    $primary_nav = wp_get_nav_menu_object(__('Primary Navigation', 'tend'));

    if (!$primary_nav) {
      $primary_nav_id = wp_create_nav_menu(__('Primary Navigation', 'tend'), array('slug' => 'primary_navigation'));
